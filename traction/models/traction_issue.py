...
class TractionIssue(models.Model):
    ...
    @api.depends(
        "issues_list_id",
        "issues_list_id.team_ids",
        "issues_list_id.team_ids.member_ids",
    )
    def _compute_allowed_user_ids(self):
        for rec in self:
            rec.allowed_user_ids = rec.issues_list_id.mapped("team_ids").mapped(
                "member_ids"
            )
    ...
    def _read_group_stage_ids(self, stages, domain, order):
        ...
        if "default_issues_list_id" in self.env.context:
            search_domain = [
                "|",
                ("issues_list_id", "=", self.env.context["default_issues_list_id"]),
            ] + search_domain
