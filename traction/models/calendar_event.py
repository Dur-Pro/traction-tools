...
class Meeting(models.Model):
    ...
    team_id = fields.Many2one(
        comodel_name="traction.team", string="Traction Team", index=True
    )
    ...
    rock_ids = fields.Many2many(
        ...
        compute="_compute_rocks",
        store=True,
        compute_sudo=True,
    )

    @api.depends("team_id", "team_id.rock_ids")
    def _compute_rocks(self):
        for rec in self:
            rocks = rec.team_id.rock_ids.filtered(
                lambda rock: rock.start_date < rec.start_date
                and rock.due_date >= rec.stop_date
            )
            rec.rock_ids = [Command.set(rocks.ids)]
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get("team_id"):
                ...
                vals["partner_ids"] = [
                    Command.link(member.partner_id.id) for member in team_id.member_ids
                ]
    ...
    def _compute_is_responsible_user(self):
        for event in self:
            event.is_responsible_user = (
                self.env.user == event.user_id
                or self.env.user == event.note_taker_user_id
    ...
    #     for item in self.agenda_item_ids.filtered(
    #         lambda rec: rec.item_type == "headline" and rec.discussed
    #     ):
